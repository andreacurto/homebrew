...
type picker struct {
	...
	descMax   int    // limite caratteri descrizione inline (0 = solo larghezza lista)
...
func (p *picker) setResult(entries []catalog.Entry, err error) {
	...
	l := list.New(items, delegate{single: p.single, selected: p.selected, labelW: w + 2, descMax: p.descMax}, p.listW(), p.availableRows())
